...
import (
	...
	"github.com/deinstapel/kube-overlay-operator/resolver"
...
func main() {
	...
	zone, ok := os.LookupEnv("ROOT_ZONE")
	if !ok {
		setupLog.Error(fmt.Errorf("ROOT_ZONE not set"), "set ROOT_ZONE for sidecar")
		os.Exit(1)
	}
	...
	reconciler := resolver.NewDNSReconciler(mgr.GetClient(), mgr.GetScheme(), namespace, zone, os.Getenv("CLUSTER_DOMAIN"))
	...
	go func() {
		if err := reconciler.ServeDNS(signal); err != nil {
			setupLog.Error(err, "problem running dns")
		}
	}()
